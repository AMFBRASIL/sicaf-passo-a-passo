...
const passosBase = SICAF_PASSOS.map((p) => ({ ...p }));
...
function clienteParaEmpresaManutencao(
  cliente: SicafPageCliente,
  painel: EmpresaGerenciarPainel,
): EmpresaData {
  return {
    clienteId: cliente.clienteId,
    sicafId: painel.sicaf?.id,
    nome: cliente.nome,
    cnpj: cliente.cnpj,
    sicaf: "ativo",
    proximoPasso: "",
    acao: { label: "", icon: Building2 },
...
function ManutencaoSidebarCard({
  ativa,
  subtitulo,
  onAtivar,
  onGerenciar,
  ...
  return (
    <Card
      className={
        ativa
          ? "shadow-soft border-emerald-500/40 bg-gradient-to-br from-emerald-500/10 to-emerald-500/5"
          : "shadow-soft border-border bg-card"
      }
    >
      <CardContent className="p-4 space-y-3">
        ...
        {ativa ? (
          <Button
            ...
            size="sm"
            variant="outline"
            className="h-8 w-full gap-1.5 rounded-full border-background/80 bg-background text-xs font-semibold hover:bg-background/90"
            onClick={onGerenciar}
          >
            Gerenciar
          ...
          <Button
            type="button"
            size="sm"
            className="h-8 w-full gap-1.5 rounded-full text-xs font-semibold"
            onClick={onAtivar}
          >
...
function SicafPage() {
  ...
  const empresaManutencao = useMemo(
    () => (cliente && painel ? clienteParaEmpresaManutencao(cliente, painel) : null),
    [cliente, painel],
  );
  ...
  return (
    <div className="w-full px-4 py-6 sm:px-6 lg:px-8 xl:px-10 2xl:px-12 sm:py-10">
      ...
      />
      {empresaManutencao && manutencaoModal && painel && (
        <ManutencaoModal
          open
          onOpenChange={(v) => !v && setManutencaoModal(null)}
          empresa={empresaManutencao}
          painel={painel}
          mode={manutencaoModal}
          diaVencimento={diaVencimentoManut}
          onAtivar={() => void recarregar()}
          onCancelar={() => void recarregar()}
