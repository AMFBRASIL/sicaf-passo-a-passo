...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (acao === "mensagem") {
      const result = await svc.enviarMensagem(usuarioId, sessaoId, String(body.texto || ""), "atendente");
      return NextResponse.json(result, { status: result.ok ? 201 : 400 });
    }
    if (acao === "sinal") {
      const result = await svc.enviarSinal(usuarioId, sessaoId, body, "atendente");
      return NextResponse.json(result, { status: result.ok ? 201 : 400 });
    }

    const result = await svc.atualizarStatus(usuarioId, sessaoId, body, "atendente");
