...
type AdminClientsService = {
  ...
  ) => Promise<{ ok: boolean; error?: string; message?: string; resumo?: unknown }>;
};
...
export async function POST(
  ...
  try {
    ...
    const result = await svc.reactivateClientCnpj(clienteId, {
      usuarioId,
      motivo: motivo || undefined,
    });
    return NextResponse.json(result, { status: result.ok ? 200 : 400 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Erro ao reativar cadastro";
