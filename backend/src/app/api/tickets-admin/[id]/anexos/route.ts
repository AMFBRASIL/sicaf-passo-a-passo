...
export async function POST(
  ...
  try {
    ...
    const result = await tickets.adicionarAnexoAdmin(
      ticketId,
      usuarioId,
      {
        ...fileResult,
    ...
    const status =
      message.includes("Token") || message.includes("Sessão") || message.includes("restrito")
        ? message.includes("restrito")
          ? 403
          : 401
        : 500;
    console.error("[TicketsAdmin:anexos]", message);
