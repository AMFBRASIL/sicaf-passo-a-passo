...
(function () {
  ...
  function applyFabVisibility() {
    // Regra solicitada: manter o botão sempre visível.
    showFab();
  }

  function syncPanelStateFromBackground() {
    chrome.runtime.sendMessage({ type: 'get-state' }, (res) => {
      if (chrome.runtime.lastError) return;
      panelOpen = !!res?.assistantSurfaceOpen;
      applyFabVisibility();
    });
  }
  ...
  function injectFloatingButton() {
    ...
    document.getElementById('cadbrasil-fab-btn').addEventListener('click', () => {
      ...
      // Mantém o botão visível mesmo com assistente aberto/fechado.
      setTimeout(syncPanelStateFromBackground, 700);
    });
